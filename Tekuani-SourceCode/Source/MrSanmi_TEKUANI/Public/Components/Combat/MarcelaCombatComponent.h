...
#pragma region Shooting System
	/**
	 * @brief Attempts to perform a shooting action.
	 *
	 * Validates shooting conditions (aiming, future ammo/cooldowns),
	 * performs a raycast from screen center, and processes hit results.
	 *
	 * This function acts as the main entry point for firing logic.
	 * Notes:
	 ...
	 * - Recoil is applied immediately after a valid shot.
	...
	/**
	 * @brief Handles logic when a hit is detected.
	 *
	 * @param Hit Result of the line trace.
	 */
	UFUNCTION()
	void OnHit(const FHitResult& Hit);
	
	/**
	 * @brief Determines whether the character is allowed to shoot.
	 *
	 * @return True if shooting conditions are met.
	 */
	bool CanShoot() const;
...
private:
	 ...
	 *
	 * @param PC Player controller used to apply camera input.
	 */
	void ApplyRecoil(APlayerController* PC);
